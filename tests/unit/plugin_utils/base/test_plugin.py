...
def test_require_aws_sdk(monkeypatch):
    ...
    base_plugin = utils_base.AWSPluginBase()
    assert base_plugin.require_aws_sdk(botocore_version=sentinel.PARAM_BOTOCORE, boto3_version=sentinel.PARAM_BOTO3) is sentinel.RETURNED_SDK
    assert require_sdk.call_args == call(botocore_version=sentinel.PARAM_BOTOCORE, boto3_version=sentinel.PARAM_BOTO3, warn=base_plugin.warn)
