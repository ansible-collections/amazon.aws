...
from ansible_collections.amazon.aws.plugins.module_utils.acm import (
    ACMServiceManager,
...
def test_acm_service_manager_import_certificate_failure_at_deletion(acm_service_mgr):
    ...
    with pytest.raises(SystemExit):
        acm_service_mgr.import_certificate(certificate=MagicMock(), private_key=MagicMock())
    acm_service_mgr.module.warn.assert_called_with("Certificate %s exists, and is not tagged. So Ansible will not see it on the next run." % arn)
...
def test_acm_service_manager_import_certificate_failure_with_arn_change(acm_service_mgr):
    ...
    acm_service_mgr.module.fail_json.assert_called_with(msg="ARN changed with ACM update, from %s to %s" % (original_arn, arn))
...
def test_acm_service_manager_get_certificates(acm_service_mgr, domain_name, arn, tags, expected):
    ...
    certificate_tags = {
        'arn:aws:1': [{'Key': 'Validated', 'Value': True}, {'Key': 'CertificateType', 'Value': 'x509'}, {'Key': 'CertificateArn', 'Value': 'arn:aws:1'}],
    ...
    acm_service_mgr.describe_certificate_with_backoff.side_effect = lambda *args, **kwargs: describe_certificates.get(args[0])
