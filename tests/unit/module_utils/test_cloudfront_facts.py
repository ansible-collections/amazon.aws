    ...
    return botocore.exceptions.ClientError(
        {
            "Error": {
                "Code": "AccessDenied",
                "Message": "User: Unauthorized operation"
            },
            "ResponseMetadata": {
...
def test_get_invalidation(cloudfront_facts_service):
    ...
    assert cloudfront_facts == cloudfront_facts_service.get_invalidation(distribution_id=distribution_id, id=cloudfront_id)
...
def test_list_distributions_by_web_acl_id(m_cloudfront_facts_keyed_list_helper, cloudfront_facts_service):
    ...
    distribution_webacl = {
        'DistributionList': {
            'Items': ["webacl_%d" % d for d in range(10)]
        }
    }
...
def test_list_origin_access_identities(m_cloudfront_paginate_build_full_result, m_cloudfront_facts_keyed_list_helper, cloudfront_facts_service):
...
@patch(MOCK_CLOUDFRONT_FACTS_KEYED_LIST_HELPER)
@patch(MODULE_NAME + "._cloudfront_paginate_build_full_result")
def test_list_distributions(m_cloudfront_paginate_build_full_result, m_cloudfront_facts_keyed_list_helper, cloudfront_facts_service):
...
@patch(MOCK_CLOUDFRONT_FACTS_KEYED_LIST_HELPER)
@patch(MODULE_NAME + "._cloudfront_paginate_build_full_result")
def test_list_invalidations(m_cloudfront_paginate_build_full_result, m_cloudfront_facts_keyed_list_helper, cloudfront_facts_service):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            [
                {'Id': 'id_1', 'Aliases': {}},
                {'Id': 'id_2', 'Aliases': {'Items': ['alias_1', 'alias_2']}}
            ],
            {
...
def test_get_distribution_id_from_domain_name(cloudfront_facts_service, distributions, streaming_distributions, domain_name, expected):
            ...
            [
                {
                    ...
                    'Enabled': 'False',
                    'Tags': {
                        'Items': [{'Name': 'tag1', 'Value': 'distribution1'}]
                    },
                    'ETag': 'abcdefgh',
                    '_ids': []
                },
                {
                    ...
                    'Enabled': 'True',
                    'Tags': {
                        'Items': [{'Name': 'tag2', 'Value': 'distribution2'}, {'Name': 'another_tag', 'Value': 'item 2'}]
                    },
                    'ETag': 'ABCDEFGH',
...
def test_summary_get_distribution_list(m_boto3_tag_list_to_ansible_dict, cloudfront_facts_service, streaming, distributions, expected):
    ...
    cloudfront_facts_service.get_etag_from_distribution_id.side_effect = lambda id, stream: [x['ETag'] for x in distributions if x['Id'] == id][0]

    cloudfront_facts_service.get_list_of_invalidation_ids_from_distribution_id = MagicMock()
    cloudfront_facts_service.get_list_of_invalidation_ids_from_distribution_id.side_effect = lambda id: [x['_ids'] for x in distributions if x['Id'] == id][0]

    cloudfront_facts_service.list_resource_tags = MagicMock()
    cloudfront_facts_service.list_resource_tags.side_effect = lambda arn: {'Tags': x['Tags'] for x in distributions if x['ARN'] == arn}
...
def test_summary(cloudfront_facts_service):
    ...
    cloudfront_facts_service.summary_get_distribution_list.side_effect = lambda x: {'called_with_true': True} if x else {'called_with_false': False}

    cloudfront_facts_service.summary_get_origin_access_identity_list = MagicMock()
    cloudfront_facts_service.summary_get_origin_access_identity_list.return_value = {'origin_access_ids': ['access_1', 'access_2']}

    expected = {
        'called_with_true': True,
        'called_with_false': False,
    ...
    cloudfront_facts_service.summary_get_distribution_list.assert_has_calls(
        [call(True), call(False)], any_order=True
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            [
                {'Id': 'some_id', 'ETag': 'some_Etag'},
                {'Id': 'another_id', 'ETag': 'another_Etag'}
            ]
        )
    ]
)
def test_summary_get_origin_access_identity_list(cloudfront_facts_service, origin_access_identities, expected):
    ...
    cloudfront_facts_service.get_origin_access_identity.side_effect = lambda x: [o['response'] for o in origin_access_identities if o['Id'] == x][0]
