...
class TestValidateOptions:
    ...
    def test_validate_valid_options(self, lookup_plugin):
        ...
        # Should not raise
        lookup_plugin._validate_options("error", "warn")
        lookup_plugin._validate_options("warn", "skip")
        lookup_plugin._validate_options("skip", "error")

    def test_validate_none_options(self, lookup_plugin):
        """Test that None options are allowed"""
        # Should not raise
        lookup_plugin._validate_options(None, None)

    @pytest.mark.parametrize(
        "on_missing,expected_error",
        [
            ("invalid", '"on_missing" must be a string and one of "error", "warn" or "skip", not invalid'),
            (["error"], '"on_missing" must be a string and one of "error", "warn" or "skip", not [\'error\']'),
            (True, '"on_missing" must be a string and one of "error", "warn" or "skip", not True'),
            (
                {"error": True},
                '"on_missing" must be a string and one of "error", "warn" or "skip", not {\'error\': True}',
            ),
        ],
    )
    def test_validate_invalid_on_missing(self, lookup_plugin, on_missing, expected_error):
        ...
        with pytest.raises(AnsibleLookupError) as exc_info:
            lookup_plugin._validate_options(on_missing, "error")
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (["warn"], '"on_denied" must be a string and one of "error", "warn" or "skip", not [\'warn\']'),
            (False, '"on_denied" must be a string and one of "error", "warn" or "skip", not False'),
        ],
    )
    def test_validate_invalid_on_denied(self, lookup_plugin, on_denied, expected_error):
        ...
        with pytest.raises(AnsibleLookupError) as exc_info:
            lookup_plugin._validate_options("error", on_denied)
        assert expected_error == str(exc_info.value)
    ...
    def test_validate_shortnames_droppath_mutually_exclusive(self, lookup_plugin):
        ...
        lookup_plugin.params = {"shortnames": True, "droppath": True}

        with pytest.raises(AnsibleLookupError) as exc_info:
            lookup_plugin._validate_options("error", "error")
