...
def _sort_listener_actions(actions: List[Dict[str, str]]) -> List[Dict[str, str]]:
    return sorted(actions, key=lambda x: (x.get("AuthenticateOidcConfig"), x.get("FixedResponseConfig"), x.get("RedirectConfig"), x.get("TargetGroupArn"), x.get("Type")))
    ...
    if new_default_actions:
        if current_default_actions and len(current_default_actions) == len(new_default_actions):
            current_actions_sorted = _sort_listener_actions(
                {k: v for k,v in x.items() if k in ["AuthenticateOidcConfig", "FixedResponseConfig", "RedirectConfig", "TargetGroupArn", "Type"]} for x in current_default_actions
            )
