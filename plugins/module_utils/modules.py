...
class AnsibleAWSModule(object):
    ...
    def __init__(self, **kwargs):
        ...
        if local_settings["check_boto3"]:
            if not HAS_BOTO3:
                self._module.fail_json(msg=missing_required_lib("botocore and boto3"))
            if not self.botocore_at_least("1.21.0"):
                self.warn("botocore < 1.21.0 is not supported or tested." "  Some features may not work.")
    ...
    def client(self, service, retry_decorator=None):
        ...
        conn = boto3_conn(self, conn_type="client", resource=service, region=region, endpoint=endpoint_url, **aws_connect_kwargs)
        return conn if retry_decorator is None else RetryingBotoClientWrapper(conn, retry_decorator)

    def resource(self, service):
        region, endpoint_url, aws_connect_kwargs = get_aws_connection_info(self, boto3=True)
        return boto3_conn(self, conn_type="resource", resource=service, region=region, endpoint=endpoint_url, **aws_connect_kwargs)
    ...
    def require_boto3_at_least(self, desired, **kwargs):
        ...
        if not self.boto3_at_least(desired):
            self._module.fail_json(msg=missing_required_lib("boto3>={0}".format(desired), **kwargs), **self._gather_versions())
    ...
    def require_botocore_at_least(self, desired, **kwargs):
        ...
        if not self.botocore_at_least(desired):
            self._module.fail_json(msg=missing_required_lib("botocore>={0}".format(desired), **kwargs), **self._gather_versions())
