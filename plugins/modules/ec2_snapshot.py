...
def _delete_snapshot(module, ec2, snapshot_id):
    if module.check_mode:
        try:
            snapshots = describe_snapshots(ec2, SnapshotIds=[(snapshot_id)])
            if not snapshots:
    ...
    try:
        changed = delete_snapshot(ec2, snapshot_id)
    except AnsibleEC2Error as e:
...
def _modify_snapshot_createVolumePermission(
    module: AnsibleAWSModule, ec2, snapshot_id: str, purge_create_vol_permission: bool
) -> None:
