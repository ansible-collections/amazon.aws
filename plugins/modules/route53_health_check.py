    ...
    health_check_config:
      ...
      contains:
        ...
        port:
          ...
          sample: 'updated'
        child_health_checks:
          description: The child health checks used for a calculated health check.
          type: list of str
          returned: When the health check exists and child health checks are configured.
          sample: ['uuid1', 'uuid2']
        health_threshold:
          description: The minimum number of healthy child health checks for a calculated health check to be considered healthy.
          type: int
...
### START - module_utils/route53.py ###

# Source: https://raw.githubusercontent.com/ansible-collections/amazon.aws/5.0.2/plugins/module_utils/route53.py
...
def create_health_check(ip_addr_in, fqdn_in, type_in, request_interval_in, port_in, child_health_checks_in, health_threshold_in):
    ...
    if type_in == 'CALCULATED':
        ...
        # Measure Latency should be configurable in the future
        health_check['MeasureLatency'] = True
