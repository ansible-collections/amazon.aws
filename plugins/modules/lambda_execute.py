...
def main():
    ...
    if invoke_params["InvocationType"] == "RequestResponse":
        ...
        if isinstance(results.get("output"), dict) and any([results["output"].get("stackTrace"), results["output"].get("errorMessage")]):
            # AWS sends back stack traces and error messages when a function failed
            # in a RequestResponse (synchronous) context.
            template = "Function executed, but there was an error in the Lambda function. " "Message: {errmsg}, Type: {type}, Stack Trace: {trace}"
            error_data = {
                # format the stacktrace sent back as an array into a multiline string
                "trace": "\n".join(
                    [" ".join([str(x) for x in line]) for line in results.get("output", {}).get("stackTrace", [])]  # cast line numbers to strings
                ),
