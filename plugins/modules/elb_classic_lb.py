...
class ElbManager(object):
    ...
    def _wait_for_elb_interface_created(self):
        ...
        filters = ansible_dict_to_boto3_filter_list({"requester-id": "amazon-elb", "description": "ELB {0}".format(self.name)})
        ...
        for listener in self.listeners:
            ...
            if proxy_ports.get(instance_port, None) is not None:
                if proxy_ports[instance_port] != proxy_protocol:
                    self.module.fail_json_aws("proxy_protocol set to conflicting values for listeners" " on port {0}".format(instance_port))
    ...
    def _set_proxy_protocol_policy(self, policy_name):
        ...
        if existing_policy is not None:
            self.module.fail_json(
                msg="Unable to configure ProxyProtocol policy. " "Policy with name {0} already exists and doesn't match.".format(policy_name),
                policy=proxy_policy,
