    ...
    if not instance_profile_setting and desired_profile_name:
        ...
        try:
            client.associate_iam_instance_profile(
                aws_retry=True, IamInstanceProfile={"Arn": determine_iam_role(desired_profile_name)}, InstanceId=instance["InstanceId"]
            )
            return True
        except (botocore.exceptions.BotoCoreError, botocore.exceptions.ClientError) as e:
...
def warn_if_public_ip_assignment_changed(instance):
    ...
    if (public_dns_name and not assign_public_ip) or (assign_public_ip and not public_dns_name):
        module.warn(
            ...
            "Whether or not to assign a public IP is determined during instance creation.".format(assign_public_ip, instance["InstanceId"])
        )
...
def warn_if_cpu_options_changed(instance):
    ...
    if threads_per_core_curr != threads_per_core:
        module.warn(
            ...
            "Assigning a number of threads per core is determined during instance creation.".format(threads_per_core_curr, threads_per_core)
        )
...
def discover_security_groups(group, groups, parent_vpc_id=None, subnet_id=None):
    ...
    if subnet_id is not None:
        try:
            ...
            module.fail_json(
                "Could not find subnet {0} to associate security groups. Please check the vpc_subnet_id and security_groups parameters.".format(subnet_id)
            )
...
def await_instances(ids, desired_module_state="present", force_wait=False):
    ...
    try:
        ...
        module.fail_json(msg="{0}. Error waiting for instances {1} to reach state {2}".format(to_native(e), ", ".join(ids), boto3_waiter_type))
    except botocore.exceptions.WaiterError as e:
        module.warn("Instances {0} took too long to reach state {1}. {2}".format(", ".join(ids), boto3_waiter_type, to_native(e)))
...
def ensure_instance_state(desired_module_state, filters):
    ...
    if desired_module_state in ("running", "started"):
        ...
        if failed:
            module.fail_json(msg="Unable to start instances: {0}".format(failure_reason), reboot_success=list(_changed), reboot_failed=failed)
        ...
        if failed:
            module.fail_json(msg="Unable to stop instances: {0}".format(failure_reason), stop_success=list(_changed), stop_failed=failed)
        ...
        if failed:
            module.fail_json(msg="Unable to restart instances: {0}".format(failure_reason), reboot_success=list(_changed), reboot_failed=failed)
        ...
        if terminate_failed:
            module.fail_json(
                msg="Unable to terminate instances: {0}".format(failure_reason), terminate_success=list(terminated), terminate_failed=terminate_failed
            )
        results = dict(
...
def enforce_count(existing_matches, module, desired_module_state):
    ...
    try:
        ...
        if current_count == exact_count:
            module.exit_json(changed=False, msg="{0} instances already running, nothing to do.".format(exact_count))
...
def ensure_present(existing_matches, desired_module_state):
    ...
    try:
        ...
        for ins in instances:
            ...
            try:
                AWSRetry.jittered_backoff(catch_extra_error_codes=["InvalidInstanceID.NotFound"],)(client.describe_instance_status)(
...
def main():
    ...
    argument_spec = dict(
        ...
        cpu_options=dict(
            type="dict", options=dict(core_count=dict(type="int", required=True), threads_per_core=dict(type="int", choices=[1, 2], required=True))
        ),
    ...
    if not module.params.get("instance_type") and not module.params.get("launch_template"):
        if module.params.get("state") not in ("absent", "stopped"):
            if module.params.get("count") or module.params.get("exact_count"):
                module.deprecate(
                    "Default value instance_type has been deprecated, in the future you must set an instance_type or a launch_template",
                    date="2023-01-01",
