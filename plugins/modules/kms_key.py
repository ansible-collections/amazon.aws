...
def camel_to_snake_grant(grant):
    ...
    if "EncryptionContextEquals" in constraints:
        result["constraints"]["encryption_context_equals"] = constraints[
            "EncryptionContextEquals"
        ]
    if "EncryptionContextSubset" in constraints:
        result["constraints"]["encryption_context_subset"] = constraints[
...
def convert_grant_params(grant, key):
    ...
    if grant.get("constraints"):
        ...
        if grant["constraints"].get("encryption_context_subset"):
            grant_params["Constraints"]["EncryptionContextSubset"] = grant[
                "constraints"
            ]["encryption_context_subset"]
        if grant["constraints"].get("encryption_context_equals"):
            grant_params["Constraints"]["EncryptionContextEquals"] = grant[
                "constraints"
            ]["encryption_context_equals"]
    return grant_params
...
def different_grant(existing_grant, desired_grant):
    if existing_grant.get("grantee_principal") != desired_grant.get(
        "grantee_principal"
    ):
        return True
    if existing_grant.get("retiring_principal") != desired_grant.get(
        "retiring_principal"
    ):
        return True
    if set(existing_grant.get("operations", [])) != set(
        desired_grant.get("operations")
    ):
        return True
...
def update_key(connection, module, key):
    ...
    changed |= ensure_enabled_disabled(
        connection, module, key, module.params["enabled"]
    )
    ...
    changed |= update_tags(
        connection, module, key, module.params["tags"], module.params.get("purge_tags")
    )
    ...
    changed |= update_key_rotation(
        connection, module, key, module.params.get("enable_key_rotation")
    )
...
def main():
    ...
    key_metadata = fetch_key_metadata(
        kms, module, module.params.get("key_id"), module.params.get("alias")
    )
