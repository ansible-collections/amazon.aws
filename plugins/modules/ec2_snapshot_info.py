...
options:
  ...
  max_results:
    ...
    - When this parameter is used, `ec2_snapshot_facts` only returns results in a single page along with a C(next_token) response element.
    - The remaining results of the initial request can be seen by sending another request with the returned C(next_token) value.
    - This value can be between 5 and 1000; if C(next_token) is given a value larger than 1000, only 1000 results are returned.
  ...
  next_token:
    ...
    - Pagination continues from the end of the previous results that returned the C(next_token) value.
    - This parameter is mutually exclusive with C(snapshot_ids)
    required: false
    type: str
notes:
...
def list_ec2_snapshots(connection, module):
    ...
    if snapshots.get('NextToken'):
        result.update(camel_dict_to_snake_dict({'NextToken': snapshots.get('NextToken')}))
